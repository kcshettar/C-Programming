...
{
	Vector_String_Ptr hVectorString = malloc(sizeof(Vector_String));

	if(hVectorString == NULL)
	return NULL;

	hVectorString->size = 0;
	hVectorString->capacity = 0;
	hVectorString->data = malloc(sizeof(String_Ptr));
	
	if(hVectorString->data == NULL)
	return NULL;

	hVectorString->data[0] = NULL;
...
Status vector_string_push_back(Vector_String_Ptr hVectorString, String_Ptr hString)
{
	...
	if(hVectorString != NULL)
	{
		if(hVectorString->size == hVectorString->capacity)
		{
			...
			{
				hVectorString->data[0] = string_init_c_string(string_c_str(hString));
				hVectorString->size++;
			}
			else
			{
